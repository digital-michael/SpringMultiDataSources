...
/**
 * Provides data source definition for augment as defined in resources/dev/audit-db.properties.
 *
...
@EnableJpaRepositories(basePackages = "com.digitalmichael.smds.springmultidatasources.augment",  // << Associates repositories in this package
        entityManagerFactoryRef = "augmentEntityManagerFactory", transactionManagerRef = "augmentTransactionManager")
public class AugmentConfig extends AbstractDataSourceConfig {

    //NOTE: by setting the name, everything else is defined. All beans are scoped to a variation of
    // this name to prevent name collisions

    //SET THIS:
    static final private String NAME = "augment";
    ...
    public LocalContainerEntityManagerFactoryBean entityManagerFactory(@Qualifier(ENTITY_MANAGER_FACTORY_BUILDER_BEAN_NAME) EntityManagerFactoryBuilder builder, @Qualifier(DATA_SOURCE_BEAN_NAME) DataSource dataSource) {
        return builder.dataSource(dataSource).packages(PACKAGES) // Entities
                .persistenceUnit(PERSISTENCE_UNIT_NAME).build();
    }
